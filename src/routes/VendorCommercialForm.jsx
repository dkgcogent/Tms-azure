...
import { vendorAPI, projectAPI, vehicleAPI, vendorCommercialAPI, apiHelpers } from '../services/api';
import SearchableDropdown from '../components/SearchableDropdown';
import DataTable from '../components/DataTable';
import './CustomerForm.css';

const VendorCommercialForm = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [vendorRes, projectRes, vehicleRes] = await Promise.all([
          vendorAPI.getAll(),
          projectAPI.getAll(),
          vehicleAPI.getAll()
        ]);
    ...
    const fetchCommercials = async () => {
      ...
      try {
        const response = await vendorCommercialAPI.getAll();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      apiHelpers.handleFormError(error, 'Vendor Commercial Form');
    }
  };
    ...
    try {
      ...
      for (let i = 0; i < totalRows; i++) {
        ...
        try {
          ...
          let resolvedCompanyId = getVal(['vendor_company_name', 'Company Name', 'Company', 'Company / ID']);
          if (resolvedCompanyId && vendors.length > 0) {
  ...
  const handleEdit = (item) => {
    ...
    if (editData.vendor_company_name && String(editData.vendor_company_name).includes(' / ')) {
  ...
  const handleBulkDelete = async (ids) => {
    ...
    try {
      await vendorCommercialAPI.bulkDelete(ids);
...
export default VendorCommercialForm;
