...
COALESCE(at.CompanyName, c.MasterCustomerName, c.Name, 'Unknown Customer') as CustomerName,
  ...
  router.post('/', upload.fields([
    ...
    try {
      ...
      if (TripType === 'Fixed') {
        ...
        console.log('🔍 CustomerID:', CustomerID, 'Type:', typeof CustomerID, 'Truthy:', !!CustomerID);
  ...
  router.put('/:id', upload.fields([
    ...
    try {
      ...
      // File verification via fs.existsSync is skipped on Vercel (serverless, no local filesystem).
      ...
      if (transactionTable === 'fixed_transactions') {
        ...
        updateQuery = `
          UPDATE adhoc_transactions SET
            ...
            TollExpenses = ?, ParkingCharges = ?, LoadingCharges = ?, UnloadingCharges = ?, OtherCharges = ?, OtherChargesRemarks = ?,
            AdvanceRequestNo = ?, AdvanceToPaid = ?, AdvanceApprovedAmount = ?, AdvanceApprovedBy = ?,
            AdvancePaidAmount = ?, AdvancePaidMode = ?, AdvancePaidDate = ?, AdvancePaidBy = ?, EmployeeDetailsAdvance = ?,
            BalanceToBePaid = ?, BalancePaidAmount = ?, Variance = ?, BalancePaidDate = ?, BalancePaidBy = ?, EmployeeDetailsBalance = ?,
            Revenue = ?, Margin = ?, MarginPercentage = ?,
            DriverAadharDoc = COALESCE(?, DriverAadharDoc), DriverLicenceDoc = COALESCE(?, DriverLicenceDoc),
            TollExpensesDoc = COALESCE(?, TollExpensesDoc), ParkingChargesDoc = COALESCE(?, ParkingChargesDoc),
            OpeningKMImage = COALESCE(?, OpeningKMImage), ClosingKMImage = COALESCE(?, ClosingKMImage),
            Remarks = ?, TripClose = ?, Status = ?, UpdatedAt = CURRENT_TIMESTAMP
        ...
        // OpeningKMImage/ClosingKMImage may come via OpeningKMImageAdhoc field names from multer
        ...
        values = [
          ...
          AdvanceToPaid || null,
          AdvanceApprovedAmount || null,
          AdvanceApprovedBy || null,
          AdvancePaidAmount || null,
          AdvancePaidMode || null,
          AdvancePaidDate || null,
          AdvancePaidBy || null,
          EmployeeDetailsAdvance || null,
          BalanceToBePaid || null,
          BalancePaidAmount || null,
          Variance || null,
          BalancePaidDate || null,
          BalancePaidBy || null,
          EmployeeDetailsBalance || null,
          Revenue || null,
          Margin || null,
  ...
  router.get('/export/adhoc', async (req, res) => {
    try {
      ...
      worksheet.columns = [
        ...
        { header: 'Extra KM', key: 'ExtraKM', width: 15 },
        { header: 'Toll Expenses', key: 'TollExpenses', width: 15 },
      ...
      adhocRows.forEach(row => {
        ...
        worksheet.addRow({
          ...
          ExtraKM: row.ExtraKM !== null && row.ExtraKM !== undefined ? row.ExtraKM : '',
          TollExpenses: row.TollExpenses || '',
          ParkingCharges: row.ParkingCharges || '',
